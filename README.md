...
* **`client/`** — the Tkinter/Pillow multiplayer GUI. `client/cli_auth.py` runs a terminal login/register handshake against the server before any window opens; `client/network_client.py` then owns a background-thread WebSocket connection for the game session itself; `client/main_gui.py` wires both into `client/ui/window/networked_game_window.py`, which renders the board and turns clicks into algebraic-notation move requests (`client/algebraic_notation.py`, `client/network_snapshot_decoder.py`).
...
* **Built, not yet wired — authentication** (`auth_service.py`, `database.py`): registration/login backed by SQLite (`aiosqlite`), `bcrypt` password hashing, and a single persistent WAL-mode connection for concurrent safety. Each module has its own unit tests, but `ws_server.py`'s connection handler doesn't call it — the game connection is unauthenticated. `client/cli_auth.py` performs this handshake over its own short-lived connection before the GUI opens, independently of the connection the game itself is played over.
