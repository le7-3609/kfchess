...
class TestAsyncGameRunner(unittest.TestCase):
    def test_tick_advances_clock_and_resolves_motion(self) -> None:
        async def scenario():
            ...
            select_fut = runner.submit_command(ClickCommand(Position(0, 0)))  # select wR at (0,0)
            move_fut = runner.submit_command(ClickCommand(Position(0, 3)))    # move to (0,3): 3 squares away
