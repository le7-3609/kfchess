...
| 7 | Shard the game authority | `domain/coordination/leases.py`, `room_ownership.py`, `room_manager.py` | Rooms are owned by lease; killing an owner loses only its rooms | **Partly built** — leases drive the room lifecycle; the gateway/authority process split does not exist |
...
**The lifecycle now runs on it.** `RoomOwnership` is built by `app_runner` for every socket-tier process — `RedisLeaseStore` with a Redis URL, `InProcessLeases` without, so the same call path runs in both configurations rather than only in the fleet one. `RoomManager.create_room` is async because an id is now *claimed* rather than generated: it is drawn locally, confirmed against the lease store, and a candidate another instance holds is discarded for the next one. That matters at the target scale rather than theoretically — five million concurrent rooms drawn from 36^6 ids collide routinely, and two instances computing one id is exactly the divergence the design forbids. A lost lease reaches `RoomManager.surrender_room`, which reaps the room *here* instead of leaving a second authority computing it; a reap or a clean shutdown releases the lease immediately, so a finished id is re-placeable at once rather than after a TTL; and `begin_draining` is now called from the SIGTERM path alongside the readiness drain, so a terminating instance refuses new rooms while renewing what it is still running. The three ownership metrics — `kfchess_rooms_owned`, `kfchess_lease_acquisitions_total`, `kfchess_lease_failovers_total` — were declared and fed by nothing; they are now fed.

*Verified:* `tests/unit/test_room_ownership_lifecycle.py` pins the lifecycle against `InProcessLeases`, including that the composition root wires it at all. Against real Redis, `test_a_room_opened_through_the_manager_is_leased_in_redis` shows a second instance refused a seated room's id and granted it after the reap, and `test_a_manager_that_loses_its_lease_stops_running_the_room` carries the failure injection through to the lifecycle: the manager stops computing the game, not merely the key expires.

*What is not built:* the gateway and game authority remain one process rather than two, which is the larger half of this step. Placement is still "whoever took the request takes the room" — `rendezvous_owner` is implemented and unused, because nothing maintains the live-instance set it would hash over, so rooms are not routed to the least-loaded instance. And failover means the id becomes acquirable, not that the game continues: a crashed instance's rooms are still lost, which is the recorded decision (Section 15) rather than a gap.
