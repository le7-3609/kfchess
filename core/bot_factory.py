...
An LLM strategy needs the network, which core/ may not import, so the client
...
from core.rules.rule_engine import ThreatValidator, EndgameValidator
from core.io.replay import ReplayWriter, ReplayEngineDecorator
from core.input.bot import PacedBotInputSource, RandomBotInputSource, StrategyBotInputSource
from core.input.bot_strategy import (
    BotStrategyInterface,
    GreedyCaptureStrategy,
    RandomMoveStrategy,
)
from core.bootstrap import build_core, CoreComponents
...
# The difficulties core/ can compose on its own, each mapped to the strategy
...
def _strategy_for(difficulty: BotDifficulty) -> BotStrategyInterface:
    """Resolve the self-contained strategies core/ can build on its own."""
    builder = _STRATEGY_BUILDERS.get(difficulty)
    if builder is None:
        raise ValueError(
            f"{difficulty} cannot be built inside core; pass an explicit strategy instead"
        )
    return builder()
